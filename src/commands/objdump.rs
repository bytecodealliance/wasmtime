...
impl Decorator<'_> {
    ...
    fn stack_map(&mut self, address: u64, list: &mut Vec<String>) {
        ...
        while let Some((addr, map)) = stack_maps.next_if(|(addr, _pos)| u64::from(*addr) <= address)
        {
            if u64::from(addr) != address {
                continue;
            }
            list.push(format!(
                "stack map: frame={}, offsets={:?}",
                map.frame_size(),
                map.offsets().collect::<Vec<_>>()
            ));
        }
    }
