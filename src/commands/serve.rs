...
/// ticking every `timeout / N` interrupts every store about once per timeout
/// window, comparable to how epoch interruption behaves. That interval is
/// clamped to `period` (so we never tick faster than needed) and to a 0.5ms
/// floor so heavy concurrency doesn't turn this into a busy loop.
#[cfg(has_mmu_interruption)]
