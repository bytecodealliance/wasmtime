...
impl ABIMachineSpec for S390xMachineDeps {
    ...
    fn gen_load_base_offset(into_reg: Writable<Reg>, base: Reg, offset: i32, ty: Type) -> Inst {
        let mem = MemArg::reg_plus_off(base, offset.into(), ir::MemFlags::trusted());
        ...
        if probe_count == 0 {
            ...
            for _ in 0..probe_count {
                ...
                insts.push(Inst::StoreImm8 {
                    imm: 0,
                    mem: MemArg::reg(stack_reg(), ir::MemFlags::trusted()),
                });
            }
        } else {
            // Explicit probe loop.
    ...
    fn gen_clobber_save(
        ...
        if let Some((first_clobbered_gpr, _)) = get_clobbered_gprs(frame_layout) {
            ...
            insts.push(Inst::StoreMultiple64 {
                ...
                mem: MemArg::reg_plus_off(stack_reg(), offset, ir::MemFlags::trusted()),
            });
            if flags.unwind_info() {
        ...
        if flags.preserve_frame_pointers() {
            insts.push(Inst::Store64 {
                rd: gpr(1),
                mem: MemArg::reg_plus_off(stack_reg(), 0, ir::MemFlags::trusted()),
            });
        }
...
fn gen_restore_gprs(
    ...
    if let Some((first, mut last)) = clobbered_gpr {
        ...
        insts.push(Inst::LoadMultiple64 {
            ...
            mem: MemArg::reg_plus_off(reg, offset, ir::MemFlags::trusted()),
        });
    }
