...
fn zkasm_get_operands<F: Fn(VReg) -> VReg>(inst: &Inst, collector: &mut OperandCollector<'_, F>) {
    match inst {
        ...
        Inst::Lui { rd, imm } => todo!(),
        Inst::LoadConst32 { rd, imm } => todo!(),
        Inst::LoadConst64 { rd, imm } => todo!(),
        Inst::Auipc { rd, imm } => todo!(),
        Inst::FpuRR { alu_op, frm, rd, rs } => todo!(),
        Inst::AluRRR { alu_op, rd, rs1, rs2 } => todo!(),
        Inst::FpuRRR { alu_op, frm, rd, rs1, rs2 } => todo!(),
        Inst::FpuRRRR { alu_op, frm, rd, rs1, rs2, rs3 } => todo!(),
        Inst::AluRRImm12 { alu_op, rd, rs, imm12 } => todo!(),
        Inst::Load { rd, op, flags, from } => todo!(),
        Inst::Store { to, op, flags, src } => todo!(),
        Inst::Args { args } => todo!(),
        Inst::Ret { rets, stack_bytes_to_pop } => todo!(),
        Inst::Extend { rd, rn, signed, from_bits, to_bits } => todo!(),
        Inst::AdjustSp { amount } => todo!(),
        Inst::Call { info } => todo!(),
        Inst::CallInd { info } => todo!(),
        Inst::ReturnCall { callee, info } => todo!(),
        Inst::ReturnCallInd { callee, info } => todo!(),
        Inst::TrapIf { test, trap_code } => todo!(),
        Inst::TrapIfC { rs1, rs2, cc, trap_code } => todo!(),
        Inst::Jal { dest } => todo!(),
        Inst::CondBr { taken, not_taken, kind } => todo!(),
        Inst::LoadExtName { rd, name, offset } => todo!(),
        Inst::LoadAddr { rd, mem } => todo!(),
        Inst::VirtualSPOffsetAdj { amount } => todo!(),
        Inst::Mov { rd, rm, ty } => todo!(),
        Inst::MovFromPReg { rd, rm } => todo!(),
        Inst::Fence { pred, succ } => todo!(),
        Inst::Udf { trap_code } => todo!(),
        Inst::Jalr { rd, base, offset } => todo!(),
        Inst::Atomic { op, rd, addr, src, amo } => todo!(),
        Inst::AtomicStore { src, ty, p } => todo!(),
        Inst::AtomicLoad { rd, ty, p } => todo!(),
        Inst::AtomicRmwLoop { offset, op, dst, ty, p, x, t0 } => todo!(),
        Inst::Select { dst, ty, condition, x, y } => todo!(),
        Inst::BrTable { index, tmp1, tmp2, targets } => todo!(),
        Inst::AtomicCas { offset, t0, dst, e, addr, v, ty } => todo!(),
        Inst::IntSelect { op, dst, x, y, ty } => todo!(),
        Inst::Icmp { cc, rd, a, b, ty } => todo!(),
        Inst::SelectReg { rd, rs1, rs2, condition } => todo!(),
        Inst::FcvtToInt { is_sat, rd, tmp, rs, is_signed, in_type, out_type } => todo!(),
        Inst::RawData { data } => todo!(),
        Inst::Unwind { inst } => todo!(),
        Inst::DummyUse { reg } => todo!(),
        Inst::FloatRound { op, rd, int_tmp, f_tmp, rs, ty } => todo!(),
        Inst::FloatSelect { op, rd, tmp, rs1, rs2, ty } => todo!(),
        Inst::Popcnt { sum, step, tmp, rs, ty } => todo!(),
        Inst::Cltz { leading, sum, step, tmp, rs, ty } => todo!(),
        Inst::Rev8 { rs, step, tmp, rd } => todo!(),
        Inst::Brev8 { rs, ty, step, tmp, tmp2, rd } => todo!(),
        Inst::StackProbeLoop { guard_size, probe_count, tmp } => todo!(),
        Inst::VecAluRRRR { op, vd, vd_src, vs2, vs1, mask, vstate } => todo!(),
        Inst::VecAluRRRImm5 { op, vd, vd_src, vs2, imm, mask, vstate } => todo!(),
        Inst::VecAluRRR { op, vd, vs2, vs1, mask, vstate } => todo!(),
        Inst::VecAluRRImm5 { op, vd, vs2, imm, mask, vstate } => todo!(),
        Inst::VecAluRR { op, vd, vs, mask, vstate } => todo!(),
        Inst::VecAluRImm5 { op, vd, imm, mask, vstate } => todo!(),
        Inst::VecSetState { rd, vstate } => todo!(),
        Inst::VecLoad { eew, to, from, flags, mask, vstate } => todo!(),
        Inst::VecStore { eew, to, from, flags, mask, vstate } => todo!(),
    }
}
