...
mod test {
    ...
    fn test_branch_lowering() {
        ...
        let v0 = pos.ins().iconst(I32, 0x1234);
        let v1 = pos.ins().iadd(arg0, v0);
        let target = pos.func.dfg.block_with_args(bb1, &[]);
        pos.ins().brnz(v1, target);
        let target = pos.func.dfg.block_with_args(bb2, &[]);
        pos.ins().jump(target);
        pos.insert_block(bb1);
        let target = pos.func.dfg.block_with_args(bb2, &[]);
        pos.ins().brnz(v1, target);
        let target = pos.func.dfg.block_with_args(bb3, &[]);
        pos.ins().jump(target);
        pos.insert_block(bb2);
        let v2 = pos.ins().iadd(v1, v0);
        let target = pos.func.dfg.block_with_args(bb2, &[]);
        pos.ins().brnz(v2, target);
        let target = pos.func.dfg.block_with_args(bb1, &[]);
        pos.ins().jump(target);
