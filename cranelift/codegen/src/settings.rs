...
pub mod detail {
    ...
    /// Helper for extracting a the bit at `offset` in `byte` as a boolean.
    fn bit_at(byte: u8, offset: u8) -> bool {
        debug_assert!(offset < 8);
        let mask = 1 << offset;
        (byte & mask) != 0
    }

    /// Check whether a bit-sized setting is the same in `first` and `second` only if it is not
    /// `default` in `first`.
    pub fn is_same_or_default_bit(bit_offset: u8, first: u8, second: u8, default: u8) -> bool {
        let first_bit = bit_at(first, bit_offset);
        first_bit == bit_at(default, bit_offset) || first_bit == bit_at(second, bit_offset)
    }

    /// Check whether a byte-sized setting is the same in `first` and `second` only if it is not
    /// `default` in `first`.
    pub fn is_same_or_default_byte(first: u8, second: u8, default: u8) -> bool {
        first == default || first == second
    }
}
...
"#
    ...
    fn compatibility() {
        ...
        let mut b = builder();
        b.enable("enable_simd")
            .expect("`enable_simd` should be a valid setting");
        let f1 = Flags::new(b);
