...
fn main() {
    ...
    // We put all ISLE-related behavior (manifest check and automatic
    // rebuild of generated code) under a feature flag `check-isle`.
    // This is for two reasons.
    //
    // First, we want to allow downstream dependents to rely on
    // Cranelift without pulling in *any* extra crates, even `sha2`
    // which we use for the manifest-based up-to-date check. They can
    // do this by specifying `default-features = false` and then not
    // enabling `check-isle`.
    //
    // Second, the "Meta deterministic check" CI job runs this build
    // script N times to ensure it produces the same output
    ...
    // specific CI job.
    #[cfg(feature = "check-isle")]
    {
        isle::maybe_rebuild_isle(crate_dir).expect("Unhandled failure in ISLE rebuild");
    }
...
mod isle {
    ...
    fn get_isle_compilations(
        ...
        let src_isa_aarch64 = make_isle_source_path_relative(
            &cur_dir,
            crate_dir.join("src").join("isa").join("aarch64"),
        );
    ...
    pub(crate) fn maybe_rebuild_isle(
        crate_dir: &std::path::Path,
