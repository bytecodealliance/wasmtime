...
impl PartialOrd for PatternInst {
        ...
        match self {
            PatternInst::MatchEqual {
                ...
                PatternInst::MatchEqual {
                    ...
                    .cmp(a2)
                    .then_with(|| b1.cmp(b2).then_with(|| ty1.cmp(ty2))),

                PatternInst::MatchInt { .. }
                | PatternInst::MatchPrim { .. }
                | PatternInst::MatchVariant { .. }
                | PatternInst::Expr { .. }
                | PatternInst::Extract { .. }
                | PatternInst::Arg { .. } => Ordering::Less,
            },
            ...
            PatternInst::MatchInt {
                ...
                PatternInst::MatchEqual { .. } => Ordering::Greater,
                PatternInst::MatchInt {
                    input: input2,
                    ty: ty2,
                    int_val: int_val2,
                    ...
                    .cmp(input2)
                    .then_with(|| ty1.cmp(ty2).then_with(|| int_val1.cmp(int_val2))),
                PatternInst::MatchPrim { .. }
                | PatternInst::MatchVariant { .. }
                | PatternInst::Expr { .. }
                | PatternInst::Extract { .. }
            ...
            PatternInst::MatchPrim {
                ...
                PatternInst::MatchEqual { .. } | PatternInst::MatchInt { .. } => Ordering::Greater,
                PatternInst::MatchPrim {
                    input: input2,
                    ty: ty2,
                    val: val2,
                    ...
                    .cmp(input2)
                    .then_with(|| ty1.cmp(ty2).then_with(|| val1.cmp(val2))),
                PatternInst::MatchVariant { .. }
                | PatternInst::Expr { .. }
                | PatternInst::Extract { .. }
                | PatternInst::Arg { .. } => Ordering::Less,
            ...
            PatternInst::MatchVariant {
                ...
                PatternInst::MatchEqual { .. }
                | PatternInst::MatchInt { .. }
                | PatternInst::MatchPrim { .. } => Ordering::Greater,
                PatternInst::MatchVariant {
                    input: input2,
                    input_ty: input_ty2,
                    arg_tys: arg_tys2,
                    variant: variant2,
                } => input1.cmp(input2).then_with(|| {
                    input_ty1
                        .cmp(input_ty2)
                        .then_with(|| arg_tys1.cmp(arg_tys2).then_with(|| variant1.cmp(variant2)))
                }),
                PatternInst::Expr { .. }
                | PatternInst::Extract { .. }
                | PatternInst::Arg { .. } => Ordering::Less,
            ...
            PatternInst::Expr {
                seq: seq1,
                output: output1,
                output_ty: output_ty1,
            } => match other {
                PatternInst::MatchEqual { .. }
                | PatternInst::MatchInt { .. }
                | PatternInst::MatchPrim { .. }
                | PatternInst::MatchVariant { .. } => Ordering::Greater,
                PatternInst::Expr {
                    seq: seq2,
                    output: output2,
                    output_ty: output_ty2,
                } => seq1.cmp(seq2).then_with(|| {
                    output1
                        .cmp(output2)
                        .then_with(|| output_ty1.cmp(output_ty2))
                }),
                PatternInst::Extract { .. } | PatternInst::Arg { .. } => Ordering::Less,
            },

            PatternInst::Extract {
                infallible: infallible1,
                inputs: inputs1,
                input_tys: input_tys1,
                output_tys: output_tys1,
                term: term1,
            } => match other {
                PatternInst::MatchEqual { .. }
                | PatternInst::MatchInt { .. }
                | PatternInst::MatchPrim { .. }
                | PatternInst::MatchVariant { .. }
                | PatternInst::Expr { .. } => Ordering::Greater,
                PatternInst::Extract {
                    infallible: infallible2,
                    inputs: inputs2,
                    input_tys: input_tys2,
                    output_tys: output_tys2,
                    term: term2,
                } => infallible1.cmp(infallible2).then_with(|| {
                    inputs1.cmp(inputs2).then_with(|| {
                        input_tys1.cmp(input_tys2).then_with(|| {
                            output_tys1.cmp(output_tys2).then_with(|| term1.cmp(term2))
                        })
                    })
                }),
                PatternInst::Arg { .. } => Ordering::Less,
            ...
            PatternInst::Arg {
                ...
                | PatternInst::MatchVariant { .. }
                | PatternInst::Expr { .. }
                | PatternInst::Extract { .. } => Ordering::Greater,
                PatternInst::Arg {
                    index: index2,
                    ty: ty2,
                } => index1.cmp(index2).then_with(|| ty1.cmp(ty2)),
            },
        }
    }
}
