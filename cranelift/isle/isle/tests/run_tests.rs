...
use cranelift_isle::{codegen, compile, lexer, parser};
use std::default::Default;

fn build(filename: &str) -> Result<String> {
    let lexer = lexer::Lexer::from_files(vec![filename])?;
    let defs = parser::parse(lexer)?;
    let mut opts: codegen::CodegenOptions = Default::default();
    opts.match_failure_tracing = false;
    compile::compile(&defs, &opts)
}
