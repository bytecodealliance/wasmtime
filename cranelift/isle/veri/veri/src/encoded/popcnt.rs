...
fn zero_extend(smt: &mut Solver<'_>, padding: usize, value: SExpr) -> SExpr {
    if padding == 0 {
        return value;
    }
    smt.list(vec![
...
pub fn popcnt(smt: &mut Solver<'_>, ty: usize, x: SExpr, id: usize) -> SExpr {
    ...
    let mut bits: Vec<_> = (0..ty)
        .map(|i| zero_extend(smt, bits_for_result - 1, smt.extract(i, i, x)))
        .collect();
