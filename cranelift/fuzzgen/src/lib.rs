...
impl<'a> FuzzGen<'a> {
    ...
    fn get_variable_of_type(
        ...
        let mut opts: Vec<
            fn(fg: &mut FuzzGen, builder: &mut FunctionBuilder, ty: Type) -> Result<Variable>,
        > = Vec::with_capacity(2);

        // Generate new
        opts.push(|fg, builder, ty| {
            let imm64 = match ty {
                I8 => fg.u.arbitrary::<i8>()? as i64,
                I16 => fg.u.arbitrary::<i16>()? as i64,
                I32 => fg.u.arbitrary::<i32>()? as i64,
                I64 => fg.u.arbitrary::<i64>()?,
                _ => unreachable!(),
            };
            let var = fg.create_var(ty)?;
