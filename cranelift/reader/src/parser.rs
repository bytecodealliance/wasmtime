...
impl<'a> Parser<'a> {
    ...
    fn parse_heap_command(&mut self) -> ParseResult<HeapCommand> {
        ...
        self.match_token(Token::Colon, "expected a ':' a heap command")?;
...
mod tests {
    ...
    fn parse_heap_commands() {
        ...
        let static_heap = parse("heap: static, size=10").unwrap();
        assert_eq!(static_heap.size, Uimm64::new(10));
        assert_eq!(static_heap.heap_type, HeapType::Static);
        let static_heap = parse("heap: dynamic, size=0x10").unwrap();
        assert_eq!(static_heap.size, Uimm64::new(16));
        assert_eq!(static_heap.heap_type, HeapType::Dynamic);
