...
unsafe fn table_grow(
    ...
    // or is a `VMGcRef`'s `r64` until we look at the table type.
    init_value: *mut u8,
) -> Result<u32> {
...
unsafe fn table_fill(
    ...
    table_index: u32,
    dst: u32,
    // NB: we don't know whether this is a `VMGcRef`'s `r64` or a pointer to a
...
// Drop a GC reference.
//
// TODO FITZGEN: rename this.
#[cfg(feature = "gc")]
unsafe fn drop_externref(instance: &mut Instance, gc_ref: *mut u8) {
