...
mod tls {
    ...
    type Tls = *const CallThreadState<'static>;
    thread_local!(static PTR: Cell<Tls> = Cell::new(ptr::null()));

    /// Configures thread local state such that for the duration of the
    /// execution of `closure` any call to `with` will yield `ptr`, unless this
    /// is recursively called again.
    pub fn set<R>(ptr: &CallThreadState<'_>, closure: impl FnOnce() -> R) -> R {
        struct Reset<'a>(&'a Cell<Tls>, Tls);

        impl Drop for Reset<'_> {
            fn drop(&mut self) {
                super::sys::register_tls(self.1);
                self.0.set(self.1);
            }
        }
