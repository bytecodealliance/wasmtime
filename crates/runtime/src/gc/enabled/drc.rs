...
use crate::{
    ...
    GcProgress, GcRootsIter, GcRuntime, MmapVec, TypedGcRef, VMExternRef, VMGcHeader, VMGcRef,
};
use anyhow::Result;
use std::{
    alloc::Layout,
...
impl DrcHeap {
    ...
    fn with_capacity(capacity: usize) -> Result<Self> {
        let heap = MmapVec::with_capacity(capacity)?;
        ...
        T: GcHeapObject,
    {
        ...
        let start = usize::try_from(start).unwrap();
        let len = std::mem::size_of::<T>();
        let slice = &mut self.heap[start..][..len];
...
unsafe impl GcHeap for DrcHeap {
    ...
    fn reset(&mut self) {
        ...
        *no_gc_count = 0;
        ...
        // TODO: we should have knobs for how much to memzero vs how much to
        // decommit. Also, an option to just not do this at all, since it is
        // purely a defense in depth strategy. Also, this should probably be
        // refactored such that you create a `GcHeap` with a slice of memory,
        // and then the thing that manages that memory (ie the pooling
        // allocator) is the thing that decides whether to decommit and all that
        // stuff.
        //
        // Furthermore, it would be neat if an entry form the pooling
        // allocator's `MemoryPool` could be used as *either* a linear memory or
        // a GC heap. This would remove the need to do capacity planning for how
        // many linear memories versus how many GC heaps ahead of
        // time. Additionally, the options should all be the same (both for the
        // runtime/pooling allocator and for the compiler) since we are doing
        // the indexed GC heaps thing.
        //
        // But all this is deferred to future pull requests.

        // Safety: we have exclusive access to the heap and its underlying pages
        // right now.
        unsafe {
            crate::sys::vm::decommit_gc_heap_pages(heap.as_mut_ptr(), heap.len())
                .expect("failed to decommit GC heap pages");
        }
    }
}
...
mod tests {
    ...
    // TODO FITZGEN: update all these tests for the new DRC implementation
