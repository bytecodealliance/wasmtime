...
impl Instance {
    ...
    pub(crate) unsafe fn set_store(&mut self, store: Option<*mut dyn Store>) {
        if let Some(store) = store {
            ...
            *self.gc_heap_base() = (*store).gc_store().gc_heap.vmctx_gc_heap_base();
            *self.gc_heap_bound() = (*store).gc_store().gc_heap.vmctx_gc_heap_bound();
            *self.gc_heap_data() = (*store).gc_store().gc_heap.vmctx_gc_heap_data();
        } else {
            assert_eq!(
