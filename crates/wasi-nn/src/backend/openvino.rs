...
impl BackendExecutionContext for OpenvinoExecutionContext {
    ...
    fn get_output(&mut self, index: u32, destination: &mut [u8]) -> Result<u32, BackendError> {
        let mut tensor = self.0.get_output_tensor_by_index(index as usize)?;
        let tensor_size = tensor.get_byte_size()?;
        if tensor_size > destination.len() {
            return Err(BackendError::NotEnoughMemory(tensor_size));
        }

        // Copy the tensor data into the destination buffer.
        destination[..tensor_size].copy_from_slice(tensor.get_data()?);
        Ok(tensor_size as u32)
    }
}
