...
pub trait Recorder {
    ...
    fn record_event_when<T, P, F>(&mut self, pred: P, f: F)
    where
        T: Into<RREvent> + fmt::Debug,
...
pub trait Replayer: Iterator<Item = RREvent> {
    ...
    /// `ReplayError::IncorrectEventVariant` if it failed to convert type safely
    #[inline]
    fn replay_event<T, F>(&mut self, f: F) -> Result<(), ReplayError>
    where
        T: TryFrom<RREvent> + fmt::Debug,
        ReplayError: From<<T as TryFrom<RREvent>>::Error>,
    ...
    #[inline]
    fn replay_event_when<T, P, F>(&mut self, pred: P, f: F) -> Result<(), ReplayError>
    where
        T: TryFrom<RREvent> + fmt::Debug,
        ReplayError: From<<T as TryFrom<RREvent>>::Error>,
...
mod tests {
    ...
    fn rr_buffers() -> Result<()> {
        ...
        replayer.replay_event(|store_event: core_wasm::HostFuncEntryEvent, _| {
