...
impl Table {
    ...
    fn _new(store: &mut StoreOpaque, ty: TableType, init: Ref) -> Result<Table> {
        ...
        // SAFETY: the `wasmtime_export` passed in here is owned by `store` as
        // it was inserted within the store above during
        // `generate_table_export`.
        let table = unsafe { Table::from_wasmtime_table(wasmtime_export, store) };
    ...
    pub fn get(&self, mut store: impl AsContextMut, index: u64) -> Option<Ref> {
        ...
        match table.get(gc_store, index)? {
            runtime::TableElement::FuncRef(f) => {
                ...
                let func = unsafe { f.map(|f| Func::from_vm_func_ref(&store, f)) };
                Some(func.into())
            }
