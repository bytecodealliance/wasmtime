...
use wasmtime_environ::{
    CompiledFunctionsIndex, CompiledModuleInfo, DefinedFuncIndex, EntityRef, FilePos, FuncIndex,
...
impl CompiledModule {
    ...
    fn module_index(&self) -> StaticModuleIndex {
        self.module.module_index.unwrap()
    }
    ...
    pub fn func_by_text_offset(&self, text_offset: usize) -> Option<(DefinedFuncIndex, u32)> {
        ...
        let (key, func_offset) = self.index.func_by_text_offset(text_offset)?;
        match key {
            FuncKey::DefinedWasmFunction(module, def_func_index) => {
                debug_assert_eq!(module, self.module_index());
                Some((def_func_index, func_offset))
            }
            _ => None,
        }
    }
