...
impl StoreOpaque {
    ...
    /// Record the event generated by `F` if `P` holds true
    pub(crate) fn record_event<P, F>(&mut self, push_predicate: P, f: F)
    where
    ...
    pub(crate) fn replay_event<P, F>(&mut self, pop_predicate: P, f: F) -> Result<()>
    where
        P: FnOnce(&ReplayMetadata) -> bool,
        F: FnOnce(RREvent, &ReplayMetadata) -> Result<(), ReplayError>,
    {
        if let Some(buf) = self.replay_buffer_mut() {
            if pop_predicate(buf.metadata()) {
                let call_event = buf.pop_event()?;
