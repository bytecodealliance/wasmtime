...
use crate::runtime::vm::{
    host_page_size, mmap::AlignedLength, HostAlignedByteCount, Mmap, MmapOffset, MmapOffsetRaw,
    MmapVec,
};
use alloc::sync::Arc;
use core::ops::Range;
...
impl MemoryImage {
    ...
    unsafe fn map_at(&self, mmap_base: MmapOffset<'_>) -> Result<()> {
    ...
    unsafe fn remap_as_zeros_at(&self, base: *mut u8) -> Result<()> {
        // TODO: port to method on mmap, similar to map_at
        self.source
            .remap_as_zeros_at(
...
pub struct MemoryImageSlot {
    ...
    /// slot.
    ///
    /// This is only used to ensure that the mmap passed in is the same one
    /// that's being worked on.
    ///
    /// Ideally we would store a reference to the mmap here, but that would
    /// currently lead to self-referential structs. So we make do with a raw
    /// offset and a runtime check.
    base: MmapOffsetRaw,
...
impl MemoryImageSlot {
    ...
    pub(crate) fn create(
        mmap_base: MmapOffset<'_>,
        accessible: HostAlignedByteCount,
        static_size: usize,
    ) -> Self {
        MemoryImageSlot {
            base: mmap_base.to_raw(),
            static_size,
            accessible,
            image: None,
            dirty: false,
    ...
    pub(crate) fn instantiate(
        ...
        let mmap_base = mmap.reconstruct_offset(self.base);
        assert!(!self.dirty);
    ...
    fn reset_with_anon_memory(&mut self) -> Result<()> {
        ...
        unsafe {
            // XXX: This is a bit vexing -- we'd eventually like to replace
            // vm::erase_existing_mapping with calls into `Mmap`, but there's no
            // way to get hold of `Mmap` from the `Drop` impl. (In general, we
            // want to allow the `Mmap` to do internal bookkeeping.)
            //
            // Some options are:
            //
            // * Store an `Arc<Mmap>` here -- in general, use `Arc<Mmap>` rather
            //   than `Mmap`. This has implications for `&mut` methods on `Mmap`.
            // * Use some kind of typestate pattern where `clear_on_drop` being
            //   set is associated with a version of `MemoryImageSlot` that has
            //   an mmap with it. Seems tricky.
            // * Always rely on explicit drops with a passed-in `&Mmap` -- ban
            //   implicit drops via a panic bomb.
            vm::erase_existing_mapping(self.base.as_mut_ptr(), self.static_size).err2anyhow()?;
        }
...
mod test {
    ...
    fn instantiate_no_image() {
        ...
        memfd
            .instantiate(&mmap, 64 << 10, None, &ty, &tunables)
            .unwrap();
        assert!(memfd.is_dirty());
    ...
    fn instantiate_image() {
        ...
        memfd
            .instantiate(&mmap, 64 << 10, Some(&image), &ty, &tunables)
            .unwrap();
        ...
        memfd
            ...
            .unwrap();
        memfd
            .instantiate(&mmap, 128 << 10, Some(&image2), &ty, &tunables)
            .unwrap();
        let slice = unsafe { mmap.slice_mut(0..65536) };
    ...
    fn dynamic() {
        ...
        {
            memfd
                .instantiate(&mmap, initial, Some(&image), &ty, &tunables)
                .unwrap();
            assert!(memfd.has_image());
