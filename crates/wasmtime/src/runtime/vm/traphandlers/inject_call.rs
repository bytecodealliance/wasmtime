...
use crate::{fiber::StoreFiberYield, vm::VMStoreContext};
...
impl InjectedCallState {
    fn inject(
        ...
        let saved_pc = core::mem::replace(pc, injected_call_trampoline as usize);
        let saved_arg0 = core::mem::replace(arg0, trampoline);
        let saved_arg1 = core::mem::replace(arg1, trampoline_arg);
        let saved_pc = if resumable {
            saved_pc
        } else {
            injected_trap_terminate as usize
        };
        // Inject arguments to the trampoline, saving the original values.
        InjectedCallState {
            pc: saved_pc,
            arg0: saved_arg0,
            arg1: saved_arg1,
...
impl VMStoreContext {
    ...
    pub(crate) fn inject_trap_handler_hostcall(
        ...
        let state = InjectedCallState::inject(pc, arg0, arg1, handler, vmctx_raw_ptr, resumable);
...
unsafe extern "C" fn injected_trap_handler(
    ...
    let store = unsafe { &mut *vm_store_context.store_mut() };
    store.with_blocking(|_store, ctx| {
        log::trace!("yielding");
        ctx.suspend(StoreFiberYield::KeepStore).unwrap();
        log::trace!("returned from yield");
    });

    log::trace!("injected_trap_handler returning");
}

/// Second trampoline injected as the *return address* from the above
/// if we have a non-resumable trap (e.g., a Wasm trap that terminates
/// the instance rather than a debug breakpoint). If the trap is
/// non-resumable, we essentially convert the signal into a yield
/// followed by the original resume-to-exception-handler; we still
/// need to properly unwind the fiber so this latter step is still
/// necessary.
