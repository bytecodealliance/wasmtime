...
fn guest_new<T>(vmctx: *mut VMOpaqueContext, ty: TableIndex) -> u32 {
    unsafe {
        handle_result(|| {
...
fn guest_close_writable<T>(vmctx: *mut VMOpaqueContext, ty: TableIndex, writer: u32, error: u32) {
    unsafe {
        handle_result(|| {
            if error != 0 {
...
fn guest_close_readable<T>(vmctx: *mut VMOpaqueContext, ty: TableIndex, reader: u32) {
    unsafe {
        handle_result(|| {
            let cx = VMComponentContext::from_opaque(vmctx);
            let instance = (*cx).instance();
...
pub(crate) extern "C" fn error_context_new<T>(
    ...
    unsafe {
        handle_result(|| {
            _ = (vmctx, memory, realloc, string_encoding, ty, address, count);
...
pub(crate) extern "C" fn error_context_debug_message<T>(
    ...
    unsafe {
        handle_result(|| {
            _ = (vmctx, memory, realloc, string_encoding, ty, handle, address);
