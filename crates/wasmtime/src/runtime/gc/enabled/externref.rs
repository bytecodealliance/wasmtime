...
use crate::{
    ...
    AsContextMut, FuncType, GcRefImpl, GcRootIndex, HeapType, ManuallyRooted, RefType, Result,
    RootSet, Rooted, StoreContext, StoreContextMut, ValRaw, ValType, WasmTy,
};
...
impl ExternRef {
    ...
    /// more details.
    ///
    /// This operation may trigger a GC.
    ///
    /// # Example
    ///
    /// ```
        ...
        T: 'static + Any + Send + Sync,
    {
            ...
            .context("unrecoverable error when allocating new `externref`")?
        {
            Ok(gc_ref) => gc_ref,
            Err(value) => {
                // On recoverable allocation failure, do a GC and then try to
                // allocate again.
                ctx.gc();
                ctx.gc_store_mut()
                    .alloc_externref(value)
                    .context("unrecoverable error when allocating new `externref`")?
                    .map_err(|_| anyhow!("failed to allocate `externref`: GC heap out of memory"))?
            }
        };
    ...
    /// Construct a new `ExternRef` asynchronously.
    ///
    /// This is the same as `ExternRef::new` but if the allocation triggers a
    /// GC, the GC is performed asynchronously, periodically yielding control to
    /// the event loop.
    ///
    /// # Panics
    ///
    /// Panics if the given context was not configured to enable async support.
    #[cfg(feature = "async")]
    pub async fn new_async<S, T>(
        mut context: impl AsContextMut<Data = S>,
        value: T,
        ...
        T: 'static + Any + Send + Sync,
    {
        ...
        assert!(
            ctx.async_support(),
            "calling `ExternRef::new_async` requires enabling async support"
        );
    ...
    /// Construct a new, manually-rooted `ExternRef` asynchronously.
    ///
    /// This is the same as `ExternRef::new_manually_rooted` but if the
    /// allocation triggers a GC, the GC is performed asynchronously,
    /// periodically yielding control to the event loop.
    ///
    /// # Panics
    ///
    /// Panics if the given context was not configured to enable async support.
    #[cfg(feature = "async")]
    pub async fn new_manually_rooted_async<S, T>(
    ...
    pub unsafe fn from_raw(mut store: impl AsContextMut, raw: u32) -> Option<Rooted<ExternRef>> {
        ...
        let gc_ref = store.gc_store_mut().clone_gc_ref(&gc_ref);
