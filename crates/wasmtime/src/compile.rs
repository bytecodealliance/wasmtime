...
use wasmtime_environ::{
    BuiltinFunctionIndex, CompiledFunctionBody, CompiledFunctionsIndex,
    CompiledFunctionsIndexBuilder, CompiledModuleInfo, Compiler, DefinedFuncIndex, FilePos,
...
pub(crate) fn build_artifacts<T: FinishedObject>(
    ...
    T,
    Option<(CompiledModuleInfo, CompiledFunctionsIndex, ModuleTypes)>,
)> {
...
pub(crate) fn build_component_artifacts<T: FinishedObject>(
    ...
    let artifacts = ComponentArtifacts {
        info,
        index: compilation_artifacts.index,
        ty,
        types,
...
impl FunctionIndices {
    ...
    fn link_and_append_code<'a>(
        ...
        let mut index_builder = CompiledFunctionsIndexBuilder::new();
        for (key, compiled_func_index) in &self.indices {
            ...
            index_builder.push_func(*key, func_loc, src_loc);
        }
...
struct Artifacts {
    ...
    index: CompiledFunctionsIndex,
}

impl Artifacts {
    /// Assuming this compilation was for a single core Wasm module, get the
    /// resulting `CompiledModuleInfo`.
    fn unwrap_as_module_info(self) -> (CompiledModuleInfo, CompiledFunctionsIndex) {
