...
use wasmtime_environ::{BuiltinFunctionIndex, I31_DISCRIMINANT, NON_NULL_NON_I31_MASK};
...
pub fn gc_ref_global_get_builtin(
    ...
    debug_assert!(ty.is_gc_heap_type());
    Ok((
        BuiltinFunctionIndex::gc_ref_global_get(),
        func_env.builtin_function_signatures.gc_ref_global_get(func),
    ))
}
...
pub fn gc_ref_global_set_builtin(
    ...
    debug_assert!(ty.is_gc_heap_type());
    Ok((
        BuiltinFunctionIndex::gc_ref_global_set(),
        func_env.builtin_function_signatures.gc_ref_global_set(func),
    ))
}
...
impl GcCompiler for DrcCompiler {
    fn translate_read_gc_reference(
        ...
        let builtin_idx = BuiltinFunctionIndex::gc();
        let builtin_sig = func_env.builtin_function_signatures.gc(builder.func);
        let (vmctx, builtin_addr) =
            func_env.translate_load_builtin_function_address(&mut builder.cursor(), builtin_idx);
        builder
            .ins()
            .call_indirect(builtin_sig, builtin_addr, &[vmctx, gc_ref]);
        builder.ins().jump(continue_block, &[]);
    ...
    fn translate_write_gc_reference(
        ...
        let builtin_idx = BuiltinFunctionIndex::drop_gc_ref();
        let builtin_sig = func_env
            .builtin_function_signatures
            .drop_gc_ref(builder.func);
        let (vmctx, builtin_addr) =
            func_env.translate_load_builtin_function_address(&mut builder.cursor(), builtin_idx);
        let old_val_pointer =
            builder
        ...
        builder
            .ins()
            .call_indirect(builtin_sig, builtin_addr, &[vmctx, old_val_pointer]);
        builder.ins().jump(continue_block, &[]);
