...
(module $one_error_conversion
  (@interface func (export \"foo\")
    ...
    impl<'a> types::UserErrorConversion for WasiCtx<'a> {
        fn errno_from_rich_error(&mut self, e: RichError) -> Result<types::Errno, wasmtime::Trap> {
...
(module $two_error_conversions
  ...
  (@interface func (export \"baz\")
    ...
    // Bodies elided.
    impl<'a> types::UserErrorConversion for WasiCtx<'a> {
        fn errno_from_rich_error(&mut self, _e: RichError) -> Result<types::Errno, wasmtime::Trap> {
            unimplemented!()
        }
        fn errno2_from_another_rich_error(
            &mut self,
            _e: AnotherRichError,
        ) -> Result<types::Errno2, wasmtime::Trap> {
            unimplemented!()
        }
    }
    ...
    impl<'a> two_error_conversions::TwoErrorConversions for WasiCtx<'a> {
        ...
        fn baz(&mut self, _: u32) -> wasmtime::Trap {
            unimplemented!()
        }
    }
}
