...
pub fn define_datatype(namedtype: &witx::NamedType) -> TokenStream {
    match &namedtype.tref {
        ...
        witx::TypeRef::Value(v) => match &**v {
            ...
            witx::Type::Variant(v) => variant::define_variant(&namedtype.name, &v),
            witx::Type::Handle(h) => handle::define_handle(&namedtype.name, &h),
            witx::Type::Builtin(b) => define_builtin(&namedtype.name, *b),
            witx::Type::Pointer(p) => define_witx_pointer(&namedtype.name, p),
            witx::Type::ConstPointer(p) => define_witx_pointer(&namedtype.name, p),
...
fn define_witx_pointer(name: &witx::Id, pointee: &witx::TypeRef) -> TokenStream {
    let ident = names::type_(name);
