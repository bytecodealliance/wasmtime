...
use ptr::{MutPtr, OwnedPtr, SharedPtr};
#[cfg(feature = "backtrace")]
...
unsafe trait ErrorExt: core::error::Error + Send + Sync + 'static {
...
impl Error {
    ...
    #[inline]
    fn from_error_ext(error: impl ErrorExt) -> Self {
...
impl<'a> OomOrDynErrorRef<'a> {
    ...
    fn as_dyn_core_error(&self) -> &'a (dyn core::error::Error + Send + Sync + 'static) {
        match *self {
            OomOrDynErrorRef::DynError(e) => {
                // Safety: invariant of this type.
                let vtable = unsafe { e.as_ref().vtable };
...
impl From<BoxedDynError> for OomOrDynError {
    fn from(boxed: BoxedDynError) -> Self {
        ...
        debug_assert_ne!(inner, Self::OOM);
        OomOrDynError { inner }
    }
}
...
impl OomOrDynError {
    ...
    const OOM: NonNull<u8> = NonNull::<OutOfMemory>::dangling().cast();
    ...
    fn unpack(&self) -> OomOrDynErrorRef<'_> {
        if self.is_oom() {
    ...
    fn unpack_mut(&mut self) -> OomOrDynErrorMut<'_> {
        if self.is_oom() {
    ...
    fn into_boxed_dyn_core_error(self) -> Box<dyn core::error::Error + Send + Sync + 'static> {
    ...
    unsafe fn downcast(self, type_id: TypeId, ret_ptr: NonNull<u8>) {
        if self.is_oom() {
