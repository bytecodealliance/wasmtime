...
pub struct Module<'a> {
    ...
    /// Track allocated global variables by type.
    ///
    /// globals_by_type[0] => offsets into `globals` representing I32 globals
    /// globals_by_type[1] => offsets into `globals` representing I64 globals
    /// globals_by_type[2] => offsets into `globals` representing F32 globals
    /// globals_by_type[3] => offsets into `globals` representing F64 globals
...
impl<'a> Module<'a> {
    ...
    /// Allocate or reuse an already-allocated global variable of the specified type.
    ///
    /// `counts` is used to track which globals have already be used:
    /// counts[0] => number of I32 globals already in use
    /// counts[1] => number of I64 globals already in use
    /// counts[2] => number of F32 globals already in use
    /// counts[3] => number of F64 globals already in use
    fn allocate_global(&mut self, counts: &mut [usize; 4], ty: ValType) -> u32 {
        let which = match ty {
            ValType::I32 => 0,
            ValType::I64 => 1,
            ValType::F32 => 2,
