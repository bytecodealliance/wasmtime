...
impl Stacks {
    ...
    ///
    /// The Wasm module imports a function `host.check_stack: [] -> []` from the
    /// host. The host can check the Wasm's understanding of its own stack
    /// against the host's understanding of the Wasm stack to find discrepency
    /// bugs.
    ///
