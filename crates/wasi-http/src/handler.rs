...
pub type Response = http::Response<UnsyncBoxBody<Bytes, ErrorCode>>;
    ...
    S: HandlerState,
{
    ...
    /// response.  See [`HandlerState::on_request_start`] for details.
    ///
    ...
    match (proxy, view) {
        ...
        (Proxy::P3(guest), ViewFn::P3(view)) => {
            ...
            let handle = pin!(async move {
                ...
                Ok(response.map(move |body| body.map_err(ErrorCode::from).boxed_unsync()))
            });
