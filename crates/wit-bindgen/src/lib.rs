...
struct Wasmtime {
    ...
    /// Maps InterfaceId -> the snake_case module name of the first import that
    /// generated types for it (used for `[implements=<I>]label` duplicates).
    implements_first_seen: HashMap<InterfaceId, String>,
    /// Maps InterfaceId -> list of linker instance names (labels) for
    /// world-level add_to_linker to iterate over.
    implements_labels: HashMap<InterfaceId, Vec<String>>,
    /// Maps InterfaceId -> the snake_case module name of the first export that
    /// generated types for it (used for `[implements=<I>]label` export duplicates).
    implements_first_seen_export: HashMap<InterfaceId, String>,
}
        ...
        for (_, import) in resolve.worlds[world].imports.iter() {
            match import {
                WorldItem::Interface { id, .. } => {
                    // Use `entry` to avoid overwriting for implements duplicates
                    // (same InterfaceId imported under multiple labels).
                    self.interface_link_options.entry(*id).or_insert_with(|| {
    ...
    fn import(&mut self, resolve: &Resolve, name: &WorldKey, item: &WorldItem) {
        ...
        match item {
            ...
            WorldItem::Interface { id, implements, .. } => {
                ...
                // Check if this is a duplicate `[implements=<I>]label` import
                // for an interface whose types were already generated by a
                // prior implements import.
                let is_implements_dup = implements.is_some()
                    && generator.generator.implements_first_seen.contains_key(id);

                if is_implements_dup {
                    // This is a subsequent import of the same interface under a
                    // different label. The shared Host trait and add_to_linker
                    // live in the first-seen module. Just record the label
                    // so world_add_to_linker can iterate over all labels.
                    let key_name = resolve.name_world_key(name);
                    self.implements_labels
                        .entry(*id)
                        .or_default()
                        .push(key_name);
                } else {
                        ...
                        .name_interface(resolve, *id, name, false)
                    {
                        ...
                        generator.generator.interface_link_options[id]
                            .write_struct(&mut generator.src);
                        generator.types(*id);
                        let key_name = resolve.name_world_key(name);
                        let is_implements = implements.is_some();
                        generator.generate_add_to_linker(*id, &key_name, is_implements);
                    ...
                    // If this is the first implements import, record its module
                    // name so subsequent duplicates can re-export from it,
                    // and record its label for world_add_to_linker.
                    if implements.is_some() {
                        self.implements_first_seen.insert(*id, snake);
                        let key_name = resolve.name_world_key(name);
                        self.implements_labels
                            .entry(*id)
                            .or_default()
        ...
        match item {
            ...
            WorldItem::Interface { id, implements, .. } => {
                ...
                let is_implements_dup = implements.is_some()
                    && generator
                        .generator
                        .implements_first_seen_export
                        .contains_key(id);
                let first_snake_export = if is_implements_dup {
                    Some(generator.generator.implements_first_seen_export[id].clone())
                } else {
                    None
                };
                if !is_implements_dup {
                ...
                if !is_implements_dup {
                    ...
                    // For dup exports, types are re-exported from the first
                    // module via `pub use`. However, resource wrapper structs
                    // like `GuestFoo<'a> { funcs: &'a Guest }` reference the
                    // local `Guest` type, so we must regenerate them here so
                    // they reference this module's `Guest`, not the first's.
                    for (_, ty_id) in resolve.interfaces[*id].types.iter() {
                        let ty_def = &resolve.types[*ty_id];
                        if matches!(ty_def.kind, TypeDefKind::Resource) {
                            if let Some(res_name) = &ty_def.name {
                                let camel = res_name.to_upper_camel_case();
                                uwriteln!(
                                    generator.src,
                ...
                let instance_name = resolve.name_world_key_with_item(name, item);
                uwrite!(
                    generator.src,
                    "
...
pub fn new<_T>(
    ...
    let _ = &mut lookup;
                ...
                let module = if is_implements_dup {
                    let first_snake = first_snake_export.as_ref().unwrap();
                    format!(
                        "
                            #[allow(clippy::all)]
                            pub mod {snake} {{
                ...
                if is_implements_dup {
                    ...
                    let dup_name = InterfaceName::Path(path);
                    self.exports.modules.push((*id, module, dup_name));
                } else {
                    self.exports
                        .modules
                ...
                if !is_implements_dup && implements.is_some() {
                    self.implements_first_seen_export.insert(*id, snake);
                }
            }
        }
...
impl Wasmtime {
    ...
    fn world_add_to_linker(
        ...
        for (interface_id, path) in self.import_interface_paths() {
            ...
            if let Some(labels) = self.implements_labels.get(&interface_id) {
                for label in labels {
                    uwriteln!(
                        self.src,
                        "{path}::add_to_linker::<T, D>(linker {options_arg}, \"{label}\", host_getter)?;"
                    );
                }
            } else {
                uwriteln!(
...
impl<'a> InterfaceGenerator<'a> {
    ...
    fn generate_add_to_linker(&mut self, id: InterfaceId, name: &str, named: bool) {
        ...
        let name_param = if named { "name: &str," } else { "" };
