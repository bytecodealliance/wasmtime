...
impl<T: WasiView> random::Host for T {
    fn get_random_bytes(&mut self, _: &mut ResourceTable, len: u64) -> anyhow::Result<Vec<u8>> {
    ...
    fn get_random_u64(&mut self, _: &mut ResourceTable) -> anyhow::Result<u64> {
...
impl<T: WasiView> insecure::Host for T {
    ...
    fn get_insecure_random_u64(&mut self, _: &mut ResourceTable) -> anyhow::Result<u64> {
        Ok(self.ctx_mut().insecure_random.sample(Standard))
    }
}

impl<T: WasiView> insecure_seed::Host for T {
    fn insecure_seed(&mut self, _: &mut ResourceTable) -> anyhow::Result<(u64, u64)> {
