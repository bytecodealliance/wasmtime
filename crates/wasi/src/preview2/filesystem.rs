...
impl HostInputStream for FileInputStream {
    ...
    fn pollable(&self) -> HostPollable {
        HostPollable::new(async { Ok(()) }) // Always immediately ready - file reads cannot block
    }
}
...
impl HostOutputStream for FileOutputStream {
    ...
    fn pollable(&self) -> HostPollable {
        HostPollable::new(async { Ok(()) }) // Always immediately ready - file writes cannot block
    }
}
...
impl HostOutputStream for FileAppendStream {
    ...
    fn pollable(&self) -> HostPollable {
        HostPollable::new(async { Ok(()) }) // Always immediately ready - file appends cannot block
    }
}
