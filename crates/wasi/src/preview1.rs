...
impl wasi_snapshot_preview1::WasiSnapshotPreview1 for WasiP1Ctx {
        ...
        match desc {
            ...
            Descriptor::Stdin { stream, .. } => {
                ...
                let read = BlockingMode::Blocking
                    .read(&mut self.as_wasi_impl(), stream, iov.len().try_into()?)
                    .await?;
    ...
    async fn poll_oneoff(
        ...
        for sub in subs.iter() {
            ...
            let p = match sub.u {
                ...
                types::SubscriptionU::FdRead(types::SubscriptionFdReadwrite {
                    ...
                    let stream = {
                        ...
                        match desc {
                            ...
                            Descriptor::File(File { fd, position, .. }) => {
                                ...
                                let fd = fd.borrowed();
                                drop(t);
                                let r = temp.ctx.as_wasi_impl().read_via_stream(fd, pos).map_err(
                                    |e| {
                                        e.try_into()
                                            .context("failed to call `read-via-stream`")
                                            .unwrap_or_else(types::Error::trap)
                    ...
                    streams::HostInputStream::subscribe(&mut temp.ctx.as_wasi_impl(), stream)
                ...
                types::SubscriptionU::FdWrite(types::SubscriptionFdReadwrite {
                    ...
                    let stream = {
                        ...
                        match desc {
                            ...
                            Descriptor::File(File {
                                ...
                                drop(t);
                                let r = if append {
                                    temp.ctx.as_wasi_impl().append_via_stream(fd).map_err(|e| {
                                        e.try_into()
                                    ...
                                    let pos = position.load(Ordering::Relaxed);
                                    temp.ctx.as_wasi_impl().write_via_stream(fd, pos).map_err(
                                        |e| {
                                            e.try_into()
                    ...
                    streams::HostOutputStream::subscribe(&mut temp.ctx.as_wasi_impl(), stream)
        ...
        struct TempResources<'a> {
            ctx: &'a mut WasiP1Ctx,
            pollables: Vec<Resource<crate::bindings::io::streams::Pollable>>,
            inputs: Vec<Resource<crate::bindings::io::streams::InputStream>>,
            outputs: Vec<Resource<crate::bindings::io::streams::OutputStream>>,
        }

        impl Drop for TempResources<'_> {
            fn drop(&mut self) {
                for p in self.pollables.drain(..) {
                    crate::bindings::io::poll::HostPollable::drop(&mut self.ctx.as_wasi_impl(), p)
                        .unwrap();
                }
