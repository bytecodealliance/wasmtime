...
fn systemtime_from(t: system_clock::Instant) -> Result<std::time::SystemTime, ErrorCode> {
    // Handle negative seconds (times before Unix epoch)
    if t.seconds < 0 {
        // For negative seconds, we need to subtract the duration
        let duration = core::time::Duration::new((-t.seconds) as u64, t.nanoseconds);
