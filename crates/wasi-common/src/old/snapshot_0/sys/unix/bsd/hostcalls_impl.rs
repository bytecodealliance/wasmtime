...
pub(crate) fn fd_advise(
    ...
    use yanix::sys::{fcntl, F_RDADVISE};
    let _ = fcntl(
        file.as_raw_fd(),
        F_RDADVISE(offset.try_into()?, len.try_into()?),
    )?;
    Ok(())
}
