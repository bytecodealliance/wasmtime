...
/// time, before the body has been handed off to hyper), the exit fires on the first such
/// yield, tearing down the store and closing the body stream after at
/// most one chunk has been queued.  The client therefore receives a truncated
/// body, consistently failing the length check in the test.
///
...
impl service::exports::wasi::http::handler::Guest for T {
    async fn handle(_request: Request) -> Result<Response, ErrorCode> {
        ...
        wit_bindgen::spawn(async move {
            ...
            // Dropping body_tx closes the stream, which eventually causes
            // P3BodyWrapper to be dropped, resolving tx_result.
        });
