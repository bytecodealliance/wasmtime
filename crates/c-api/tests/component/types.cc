...
(component
  ...
  EXPECT_EQ(f1.params_count(store), 0);
  EXPECT_EQ(f1.params(store).size(), 0);
  EXPECT_FALSE(f1.result(store).has_value());

  EXPECT_EQ(f2.params_count(store), 1);
  auto params = f2.params(store);
  EXPECT_EQ(params.size(), 1);
  EXPECT_EQ(params[0].first, "x");
  EXPECT_TRUE(params[0].second.is_u32());
  auto result = f2.result(store);
