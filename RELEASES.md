...
- Support for `Config::strategy` has been removed from the `wasmtime` crate
  since it's easy to cause problems and useless.
  [#4188](https://github.com/bytecodealliance/wasmtime/pull/4188).
...
- Clarify that SSE 4.2 (and prior) is required for running WebAssembly code with
  simd support enabled on x86_64.
...
- Support for the SIMD proposal on both x86_64 and AArch64 has improved. On
  x86_64, all SIMD opcodes are now supported.
...
_This version ended up not getting a full release_
