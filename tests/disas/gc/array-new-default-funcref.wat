;;! target = 'x86_64'
;;! test = 'optimize'
;;! flags = '-Wgc'

(module
  (type $a (array (mut funcref)))

  (func $fill (param $len i32) (result (ref $a))
    (array.new_default $a (local.get $len))
  )
)
;; function u0:0(i64 vmctx, i64, i32) -> i32 tail {
;;     ss0 = explicit_slot 4, align = 4
;;     region0 = 8 "VMContext+0x8"
;;     region1 = 67108888 "VMStoreContext+0x18"
;;     region2 = 32 "VMContext+0x20"
;;     region3 = 872415232 "VMCopyingHeapData+0x0"
;;     region4 = 872415236 "VMCopyingHeapData+0x4"
;;     region5 = 40 "VMContext+0x28"
;;     region6 = 1677721600 "TypeIdsArray+0x0"
;;     region7 = 67108896 "VMStoreContext+0x20"
;;     region8 = 536870912 "GcHeap"
;;     region9 = 67108904 "VMStoreContext+0x28"
;;     region10 = 1543503872 "Stack(ss0)"
;;     gv0 = vmctx
;;     gv1 = load.i64 notrap aligned readonly can_move region0 gv0+8
;;     gv2 = load.i64 notrap aligned region1 gv1+24
;;     sig0 = (i64 vmctx, i32, i32, i32, i32) -> i32 tail
;;     sig1 = (i64 vmctx, i64) -> i64 tail
;;     fn0 = colocated u805306368:24 sig0
;;     fn1 = colocated u805306368:25 sig1
;;     stack_limit = gv2
;;
;;                                 block0(v0: i64, v1: i64, v2: i32):
;; @001f                               v4 = uextend.i64 v2
;;                                     v99 = iconst.i64 2
;;                                     v100 = ishl v4, v99  ; v99 = 2
;; @001f                               v7 = iconst.i64 32
;; @001f                               v8 = ushr v100, v7  ; v7 = 32
;; @001f                               trapnz v8, user18
;; @001f                               v3 = iconst.i32 20
;;                                     v106 = iconst.i32 2
;;                                     v107 = ishl v2, v106  ; v106 = 2
;; @001f                               v10 = uadd_overflow_trap v3, v107, user18  ; v3 = 20
;; @001f                               v11 = load.i64 notrap aligned readonly can_move region2 v0+32
;; @001f                               v12 = load.i32 notrap aligned region3 v11
;; @001f                               v13 = load.i32 notrap aligned region4 v11+4
;; @001f                               v19 = uextend.i64 v12
;; @001f                               v14 = uextend.i64 v10
;; @001f                               v15 = iconst.i64 15
;; @001f                               v17 = iadd v14, v15  ; v15 = 15
;; @001f                               v16 = iconst.i64 -16
;; @001f                               v18 = band v17, v16  ; v16 = -16
;; @001f                               v20 = iadd v19, v18
;; @001f                               v21 = uextend.i64 v13
;; @001f                               v22 = icmp ule v20, v21
;; @001f                               brif v22, block2, block3
;;
;;                                 block2:
;;                                     v115 = iconst.i32 15
;;                                     v116 = iadd.i32 v10, v115  ; v115 = 15
;;                                     v119 = iconst.i32 -16
;;                                     v120 = band v116, v119  ; v119 = -16
;;                                     v122 = iadd.i32 v12, v120
;; @001f                               store notrap aligned region3 v122, v11
;;                                     v137 = iconst.i32 -1476395002
;;                                     v138 = load.i64 notrap aligned readonly can_move region0 v0+8
;;                                     v139 = load.i64 notrap aligned readonly can_move region7 v138+32
;; @001f                               v36 = iadd v139, v19
;; @001f                               store user2 region8 v137, v36  ; v137 = -1476395002
;;                                     v140 = load.i64 notrap aligned readonly can_move region5 v0+40
;;                                     v141 = load.i32 notrap aligned readonly can_move region6 v140
;; @001f                               store user2 region8 v141, v36+4
;; @001f                               store user2 region8 v120, v36+8
;; @001f                               jump block4(v12, v36)
;;
;;                                 block3 cold:
;; @001f                               v23 = iconst.i32 -1476395002
;; @001f                               v24 = load.i64 notrap aligned readonly can_move region5 v0+40
;; @001f                               v25 = load.i32 notrap aligned readonly can_move region6 v24
;; @001f                               v26 = iconst.i32 16
;; @001f                               v27 = call fn0(v0, v23, v25, v10, v26)  ; v23 = -1476395002, v26 = 16
;; @001f                               v28 = load.i64 notrap aligned readonly can_move region0 v0+8
;; @001f                               v29 = load.i64 notrap aligned readonly can_move region7 v28+32
;; @001f                               v30 = uextend.i64 v27
;; @001f                               v31 = iadd v29, v30
;; @001f                               jump block4(v27, v31)
;;
;;                                 block4(v41: i32, v42: i64):
;;                                     v98 = stack_addr.i64 ss0
;;                                     store notrap aligned region10 v41, v98
;; @001f                               v43 = iconst.i64 16
;; @001f                               v44 = iadd v42, v43  ; v43 = 16
;; @001f                               store.i32 user2 region8 v2, v44
;; @001f                               trapz v41, user16
;;                                     v142 = load.i64 notrap aligned readonly can_move region0 v0+8
;;                                     v143 = load.i64 notrap aligned readonly can_move region7 v142+32
;; @001f                               v47 = uextend.i64 v41
;; @001f                               v50 = iadd v143, v47
;; @001f                               v52 = iadd v50, v43  ; v43 = 16
;; @001f                               v53 = load.i32 user2 readonly region8 v52
;; @001f                               v54 = uextend.i64 v53
;; @001f                               v60 = icmp.i64 ugt v4, v54
;; @001f                               trapnz v60, user17
;; @001f                               v77 = load.i64 notrap aligned region9 v142+40
;; @001f                               v65 = iconst.i64 20
;; @001f                               v66 = iadd v50, v65  ; v65 = 20
;; @001f                               v79 = uadd_overflow_trap v66, v100, user2
;; @001f                               v78 = iadd v143, v77
;; @001f                               v80 = icmp ugt v79, v78
;; @001f                               trapnz v80, user2
;; @001f                               v45 = iconst.i64 0
;; @001f                               v81 = call fn1(v0, v45), stack_map=[i32 @ ss0+0]  ; v45 = 0
;; @001f                               v85 = icmp.i64 eq v4, v45  ; v45 = 0
;; @001f                               v82 = ireduce.i32 v81
;; @001f                               v5 = iconst.i64 4
;; @001f                               v83 = iadd v66, v100
;; @001f                               brif v85, block6, block5(v66)
;;
;;                                 block5(v86: i64):
;; @001f                               store.i32 notrap aligned little region8 v82, v86
;;                                     v144 = iconst.i64 4
;;                                     v145 = iadd v86, v144  ; v144 = 4
;; @001f                               v89 = icmp eq v145, v83
;; @001f                               brif v89, block6, block5(v145)
;;
;;                                 block6:
;; @0022                               jump block1
;;
;;                                 block1:
;;                                     v91 = load.i32 notrap aligned region10 v98
;; @0022                               return v91
;; }
