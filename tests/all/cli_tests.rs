...
fn config_cli_flag() -> Result<()> {
    ...
    let output = run_wasmtime(&[
        ...
        "get_f64",
        "-W",
        "max-wasm-stack=1", // should override TOML value 65536 specified above and execution should fail
        wasm.path().to_str().unwrap(),
    ]);
    assert!(output
        .unwrap_err()
    ...
    assert!(output.unwrap_err().to_string().contains(
        "unknown field `invalid_table`, expected one of `optimize`, `codegen`, `debug`, `wasm`, `wasi`"
    ));
