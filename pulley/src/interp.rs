...
impl Vm {
    ...
    unsafe fn run(&mut self, pc: NonNull<u8>) -> Result<(), NonNull<u8>> {
        ...
        #[cfg(pulley_tail_calls)]
        let result = tail_loop::run(self, &mut bytecode);
        #[cfg(not(pulley_tail_calls))]
        let result = match_loop::run(self, &mut bytecode);
        match result {
...
fn ret(state: &mut MachineState, pc: &mut UnsafeBytecodeStream) -> ControlFlow<Done> {
    let lr = state[XReg::lr];
...
fn push_frame(state: &mut MachineState, _pc: &mut UnsafeBytecodeStream) -> ControlFlow<Done> {
...
fn pop_frame(state: &mut MachineState, _pc: &mut UnsafeBytecodeStream) -> ControlFlow<Done> {
