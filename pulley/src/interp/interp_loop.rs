...
define_opcode_handlers! {
    ...
    fn extended(state: &mut MachineState, pc: &mut UnsafeBytecodeStream, opcode: ExtendedOpcode) {
        match opcode {
            ...
            ExtendedOpcode::CallIndirectHost => {
                let (sig,) = crate::decode::unwrap_uninhabited(crate::decode::operands::call_indirect_host(pc));
                call_indirect_host(state, pc, sig)
            }
        }
    }
}
...
fn call_indirect_host(
    ...
    let raw = state[XReg::x0].get_ptr::<u8>();
    let mut n = 0;
    let mut arg = 1;
