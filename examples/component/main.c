    ...
    size_t nargs, wasmtime_component_val_t *results, size_t nresults)
{
  ...
  float res = args[2].payload.enumeration.discriminant == 0 ?
    args[0].payload.f32 + args[1].payload.f32 : args[0].payload.f32 * args[1].payload.f32;
  results[0].kind = WASMTIME_COMPONENT_KIND_F32;
  results[0].payload.f32 = res;
  return NULL;
}
...
int main() {
  ...
  // `multiply` only uses types without additional owned data, that can live on the stack
  ...
  wasmtime_error_t* error = wasmtime_component_linker_define_func(
    linker, "host", 4, "multiply", 8, mult_param_types, 2,
    &f32_result_type, 1, mult, (void*)multiply_data, NULL);
  if (error)
    exit_with_error("failed to define function multiply", error, NULL);

  // `apply` uses an enum, which needs additional data, use a wasmtime_component_type_vec_t
  ...
  wasmtime_component_string_vec_new_uninitialized(&apply_param_types.data[2].payload.enumeration, 2);
  wasm_name_new_from_string(&apply_param_types.data[2].payload.enumeration.data[0], "add");
  wasm_name_new_from_string(&apply_param_types.data[2].payload.enumeration.data[1], "multiply");
  error = wasmtime_component_linker_define_func(
    linker, "host", 4, "apply", 5, apply_param_types.data, apply_param_types.size,
    &f32_result_type, 1, apply, (void*)apply_data, NULL);
  if (error)
  ...
  FILE *file = fopen("target/wasm32-unknown-unknown/debug/guest.wasm", "rb");
  if (!file) {
  ...
  // Compile.
  // Note that the binary should be a component (not a plain module), typically built by running
  // `cargo component build -p example-component-wasm --target wasm32-unknown-unknown`
  // this will therefore fail if only the README instructions are followed
  printf("Compiling component...\n");
  wasmtime_component_t* component;
  error = wasmtime_component_from_binary(engine, (uint8_t *)binary.data, binary.size, &component);
  if (error)
    exit_with_error("failed to build component", error, NULL);
  wasm_byte_vec_delete(&binary);

  wasmtime_store_t *store = wasmtime_store_new(engine, (void*)context_data, NULL);
  ...
  bool ok = wasmtime_component_instance_get_func(instance, context, func_name1, strlen(func_name1), &convert1);
  if (!ok)
    exit_with_error("function convert-celsius-to-fahrenheit not found", NULL, NULL);
  wasmtime_component_func_t* convert2;
  const char* func_name2 = "convert";
  ok = wasmtime_component_instance_get_func(instance, context, func_name2, strlen(func_name2), &convert2);
  if (!ok)
  ...
  error = wasmtime_component_func_call(convert1, context, &param_val, 1, &result_val, 1, &trap);
  if (error != NULL || trap != NULL)
    exit_with_error("failed to call function convert-celsius-to-fahrenheit", error, trap);
  ...
  error = wasmtime_component_func_call(convert2, context, t, 1, result, 1, &trap);
  if (error != NULL || trap != NULL)
