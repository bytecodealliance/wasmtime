...
fuzz_target!(|data: &[u8]| {
    let Ok((seed, ops)) = postcard::from_bytes::<(u64, ExceptionOps)>(data) else {
        return;
    };
    ...
    let _ = exception_ops(config, ops);
});
...
fuzz_mutator!(|data: &mut [u8], size: usize, max_size: usize, seed: u32| {
    ...
    let mut tuple: (u64, ExceptionOps) = from_bytes(&data[..size]).ok().unwrap_or_default();
    ...
    if session.mutate(&mut tuple).is_ok() {
        loop {
            ...
            // Attempt to shrink scenarios if encoding fails (e.g., buffer too small)
            if tuple.1.pop() {
                continue;
            }
